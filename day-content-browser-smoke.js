...
const server = http.createServer((req, res) => {
  try {
    const url = new URL(req.url, 'http://127.0.0.1'), relative = url.pathname === '/' ? 'index.html' : decodeURIComponent(url.pathname.slice(1)), file = path.resolve(root, relative);
...
function verify(result, label) {
  assert.equal(result.error, undefined, label); assert.equal(result.edition, true, label); assert.ok(result.articles >= 1, label); assert.ok(result.claims >= 1, label); assert.ok(result.sources >= 1, label);
...
(async () => {
  ...
  process.stdout.write(`PASS fresh install e offline: ${online.articles} articoli, ${online.sources} fonti, parola completa\n`);
