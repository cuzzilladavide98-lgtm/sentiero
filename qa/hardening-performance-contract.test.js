...
test('peso di avvio resta sotto i budget misurati della candidata', () => {
  ...
  assert.match(html, /rel="preload" href="\.\/sentiero-app\.js\?v=60\.274\.3" as="script"/);
  assert.match(html, /<script src="\.\/sentiero-app\.js\?v=60\.274\.3"><\/script>/);
...
test('il Service Worker attiva la release solo con il runtime principale completo', () => {
  assert.match(sw, /const CORE_ASSETS = \['\.\/', '\.\/index\.html', '\.\/manifest\.json', '\.\/sentiero-app\.js\?v=60\.274\.3'\]/);
  assert.match(sw, /Promise\.all\(CORE_ASSETS\.map/);
  assert.match(sw, /throw new Error\('core asset: '/);
  assert.match(sw, /sentiero-day\.mjs\?v=60\.274\.3/);
