...
const release = { build: 'v60S.274.5', version: 'v60S.274.5 · 2026-09-04', generation: 274005, cache: 'sentiero-v60s-274-5' };
...
async function runScenario(scenario, appPort) {
  ...
  try {
    ...
    await cdp.send('Runtime.evaluate', { expression: `(async()=>{window.__finalRegistration={done:false};try{const registration=await navigator.serviceWorker.register('/sw.js?v=60.274.5',{scope:'/',updateViaCache:'none'});const limit=Date.now()+20000;while((!navigator.serviceWorker.controller||!navigator.serviceWorker.controller.scriptURL.includes('/sw.js'))&&Date.now()<limit)await new Promise(resolve=>setTimeout(resolve,100));window.__finalRegistration={done:true,scriptURL:navigator.serviceWorker.controller&&navigator.serviceWorker.controller.scriptURL};location.href='/index.html?upgrade=${scenario.id}';}catch(error){window.__finalRegistration={done:true,error:String(error&&error.stack||error)}}})()` });
