...
const distributedSnapshot = JSON.parse(fs.readFileSync(path.join(root, 'assets', 'giornale', 'latest.json'), 'utf8'));
const expectedArticles = distributedSnapshot.edition.articles.filter(article => Number(article.importance) >= 52).length;
...
const server = http.createServer((req, res) => {
  try {
    ...
    const isOfflineAsset = url.pathname === '/assets/giornale/latest.json' || url.pathname === '/assets/parole-giorno-v1.json';
    if (isOfflineAsset && !isOnline) { res.writeHead(404, { 'Cache-Control': 'no-store' }); res.end(); return; }
...
function runChrome(profile, url) {
  return new Promise((resolve, reject) => {
    const args = ['--headless', '--disable-gpu', '--no-first-run', '--disable-background-networking', '--disable-default-apps', '--disable-extensions', '--disable-sync', '--metrics-recording-only', '--window-size=390,700', '--force-device-scale-factor=2', `--user-data-dir=${profile}`, '--virtual-time-budget=60000', '--dump-dom', url];
    const child = spawn(chrome, args, { windowsHide: true }); let stdout = '', stderr = '';
    child.stdout.on('data', chunk => { stdout += chunk; }); child.stderr.on('data', chunk => { stderr += chunk; });
    const timer = setTimeout(() => { child.kill(); reject(new Error('Chrome timeout')); }, 120000);
...
function verify(result, label) {
  assert.equal(result.error, undefined, label); assert.equal(result.edition, true, label); assert.equal(result.articles, expectedArticles, label + ': articoli distribuiti non preservati'); assert.ok(result.claims >= 1, label); assert.ok(result.sources >= 1, label);
